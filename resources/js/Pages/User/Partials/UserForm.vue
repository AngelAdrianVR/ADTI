<template>
    <form @submit.prevent="$emit('submit')"
        ...
        <h1 class="font-bold ml-2 col-span-full">{{ isEdit ? 'Editar usuario' : 'Crear usuario' }}</h1>
        
        <div v-if="!isEdit" class="col-span-full text-sm px-2 py-1 bg-[#F2F2F2] rounded-sm text-[#6D6E72] flex space-x-2">
        ...
        <div>
            ...
            <el-select filterable v-model="form.civil_state" placeholder="Seleccione"
                no-data-text="No hay opciones registradas" no-match-text="No se encontraron coincidencias" class="w-full">
        ...
        <!-- NUEVO CAMPO: Turno / Horario -->
        <div>
            <InputLabel value="Turno de trabajo*" />
            <el-select filterable v-model="form.org_props.work_shift" placeholder="Seleccione el turno" class="w-full">
                <el-option v-for="shift in workShifts" :key="shift" :label="shift" :value="shift" />
            </el-select>
            <InputError :message="form.errors['org_props.work_shift']" />
        </div>
        ...
        <template v-if="$page.props.auth.user.permissions.includes('Ver sueldos')">
            <div>
        ...
        <div class="col-span-full">
            ...
            <p class="text-xs text-gray-500 mt-1">
                Selecciona los usuarios que estarán bajo la supervisión de este {{ isEdit ? 'usuario' : 'nuevo usuario' }}.
            </p>
        ...
        <DialogModal :show="showJobPositionModal" @close="showJobPositionModal = false" maxWidth="lg">
            ...
            <template #footer>
                <div class="flex items-center justify-end space-x-1">
                    <CancelButton @click="showJobPositionModal = false" :disabled="auxForm.processing">Cancelar</CancelButton>
        ...
        <DialogModal :show="showDepartmentModal" @close="showDepartmentModal = false" maxWidth="lg">
            ...
            <template #footer>
                <div class="flex items-center justify-end space-x-1">
                    <CancelButton @click="showDepartmentModal = false" :disabled="auxForm.processing">Cancelar</CancelButton>
...
export default {
    ...
    methods: {
        ...
        clearImage() {
            ...
            this.form.selectedImage = null; // Aplicado al form si está en modo edit
        },
    }
}
</script>
