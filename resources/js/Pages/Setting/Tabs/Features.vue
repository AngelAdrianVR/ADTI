...
const store = () => {
    form.post(route('features.store'), {
        onSuccess: () => {
            ElNotification.success({ title: 'Éxito', message: 'Característica creada correctamente' });
...
const update = () => {
    form.put(route('features.update', currentItem.value.id), {
        onSuccess: () => {
            ElNotification.success({ title: 'Éxito', message: 'Característica actualizada correctamente' });
            showModal.value = false;
            form.reset();
        },
    });
...
const deleteItem = (item) => {
    router.delete(route('features.destroy', item.id), {
        onSuccess: () => {
            ElNotification.success({ title: 'Éxito', message: 'Característica eliminada' });
        },
    });
};
</script>
...
<template>
    <div class="py-6 animate-fade-in">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
                <h2 class="text-lg font-bold text-[#1676A2]">Características</h2>
                <p class="text-xs text-[#6D6E72]">Define atributos reutilizables para los productos (ej. Color, Tamaño).</p>
            </div>
            <button 
                v-if="$page.props.auth.user.permissions.includes('Crear caracteristicas')"
                @click="openCreate"
            ...
            >
                <i class="fa-solid fa-plus"></i> Nueva Característica
            </button>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 text-[#6D6E72] uppercase text-xs font-semibold">
                        <tr>
                            <th class="px-6 py-3">Nombre de la Característica</th>
                            <th class="px-6 py-3 text-right">Acciones</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="item in features" :key="item.id" class="hover:bg-gray-50 transition-colors group">
        ...
        <DialogModal :show="showModal" @close="showModal = false" maxWidth="md">
            <template #title>
                <span class="font-bold text-[#1676A2]">{{ editFlag ? 'Editar Característica' : 'Nueva Característica' }}</span>
            </template>
            <template #content>
                <div class="space-y-4">
                    <div>
                        <InputLabel value="Nombre *" />
                        <TextInput v-model="form.name" class="w-full mt-1 border-gray-300 focus:border-[#1676A2] focus:ring-[#1676A2]" placeholder="Ej. Material, Color" autofocus />
