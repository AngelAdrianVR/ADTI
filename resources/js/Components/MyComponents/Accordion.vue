<template>
    <div>
        <div @click.prevent="toggleAccordion" :aria-expanded="accordionOpen" :aria-controls="`accordion-text-${id}`"
            :title="title" class="flex items-center space-x-3 px-3 rounded-lg mb-2 ml-2 py-1 hover:bg-[#c5c5c5]"
            :class="{
            ...
            <slot name="trigger">
                ...
                <button :id="`accordion-title-${id}`"
                    class="w-full text-start flex justify-between text-xs rounded-md py-1 font-bold">
