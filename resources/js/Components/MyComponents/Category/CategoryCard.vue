<template>
    <div class="border border-grayD9 px-4 py-3 rounded-[10px] self-start">
        <header class="flex items-center justify-end space-x-1">
            <button type="button" @click="$inertia.visit(route('categories.edit', category.id))" class="size-6 rounded-full bg-grayED flex items-center justify-center text-primary">
            ...
            <el-popconfirm confirm-button-text="Si" cancel-button-text="No" icon-color="#0355B5"
                    title="Se borrarán subcategorias y productos relacionados. ¿Continuar?" @confirm="$inertia.visit(route('categories.edit', category.id))">
                    <template #reference>
                        <button type="button" class="size-6 rounded-full bg-grayED flex items-center justify-center text-primary">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="size-4">
                                <path stroke-linecap="round" stroke-linejoin="round"
